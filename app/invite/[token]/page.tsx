...
import { Users, Link as LinkIcon, CheckCircle, XCircle, Loader2, ArrowRight } from 'lucide-react';
import { Button } from '@/components/ui/Button';
import { Badge } from '@/components/ui/Badge';
import { toast } from '@/components/ui/Toast';
import { createClient } from '@/lib/supabase/client';
import type { TeamInvitation } from '@/types';
import { getInvitationByToken, acceptInvitation } from '@/lib/store/team-invitations';
...
export default function InviteByTokenPage() {
  ...
  const checkAuthAndLoadInvitation = async () => {
    try {
      ...
      // Charger l'invitation par token
      ...
      setTeamName((invitationData as any).teams?.name || 'Une équipe');

      // Si l'utilisateur est connecté, vérifier s'il n'est pas déjà membre
      if (user) {
      ...
      <div className="max-w-md mx-auto p-8 bg-var(--papyrus-surface) rounded-2xl border border-var(--papyrus-border) shadow-lg">
        ...
        <div className="space-y-4">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <h3 className="font-medium text-green-900 mb-1">🎉 Invitation ouverte</h3>
            <p className="text-sm text-green-800">
              Cette invitation a été créée pour permettre à toute personne avec ce lien de rejoindre l'équipe.
            </p>
          </div>

          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h3 className="font-medium text-blue-900 mb-1">En rejoignant cette équipe :</h3>
